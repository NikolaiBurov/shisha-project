...
class TranslationsHelper
{
    ...
    public function languangeMapper(string $language, $products, Request $request)
    {
        ...
        foreach ($products as $item => $data) {
            if (!$data->translations->isEmpty()) {
                ...
                $entities[$data->id]['flavour_variations'] = array_values(
                    array_filter(
                        array_map(function ($variations) use ($data) {
                ...
                if (!is_null($data->image_gallery) && !$data->image_gallery->isEmpty()) {
                    $entities[$data->id]['image_gallery'] = ImageService::multipleImagesAbsolutePath(
                        $entities[$data->id]['image_gallery'],
                        $request
                    );
                }
            }
     ...
     * @return array
     */
    public function translateFilteredResults(
        $filters,
        $language,
        Request $request,
        ...
        foreach ($filters as $index => $fitler) {
            ...
            $filters[$index]['short_description'] = $fitler->getTranslatedAttribute(
                'short_description',
                $language,
                'bg'
            );
